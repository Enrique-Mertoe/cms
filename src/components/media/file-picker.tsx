...
export function FilePicker({
  ...
  const filteredFiles = allowedTypes.length > 0
    ? files.filter(file => file.isDirectory || !file.type || allowedTypes.includes(file.type))
    : files;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[900px] max-h-[80vh] flex flex-col">
        ...
        {/* Toolbar */}
        <div className="border-b border-gray-200 bg-gray-50 px-4 py-2 flex flex-wrap items-center justify-between gap-3 rounded-md mb-4">
          {/* Navigation Controls */}
          ...
          {isLoading ? (
            ...
            <div className={viewMode === 'grid' 
              ...
              {filteredFiles.map(file => {
                ...
                const isDisabled = !file.isDirectory && allowedTypes.length > 0 && file.type && !allowedTypes.includes(file.type);
                ...
                return (
                  ...
                  >
                    {viewMode === 'grid' ? (
                      ...
                      <>
                        {file.type?.startsWith('image/') && !file.isDirectory ? (
                          <div className="w-8 h-8 bg-gray-100 rounded-lg overflow-hidden mr-3 flex-shrink-0">
                            <img 
                              src={`/media${file.path}`} 
                              alt={file.name}
                              className="w-full h-full object-cover"
